...
\textbf{4. Metabolic-Regulatory Unified Modeling (Original):} By unifying metabolic and regulatory networks through signal type assignment, decision points emerge from the topology dynamics as quantitative thresholds. The formalism computes commitment thresholds directly from network structure: $M_{\text{commit}} = \theta(t) + W_s((p_s,t))$, where topology determines when irreversible decisions occur. For \textit{B. subtilis} sporulation, the formalism provides a mechanistic account of the Fujita and Losick~\cite{fujita2005} observation. Fujita established that $\sigma_H$ is a necessary condition (Fig.~4: $\sigma_H$-null + Spo0A* $\to 0\%$) and that timing of Spo0A$\sim$P accumulation determines outcome (Fig.~2C/D: abrupt $\approx 5\%$, gradual $\approx 52\%$). The SHPN model provides the mechanistic explanation: $T_{\mathrm{septation}}$ is gated by $[\sigma_H]^2$ through the $\Gamma = (K, n, \varepsilon)$ threshold architecture, identifying a $\sigma_H$ commitment separatrix at $\approx 1.60\,\mu$M (Sweep~A). Gradual phosphorelay accumulation allows the $\sigma_H$ loop to ramp across this separatrix; an abrupt pulse fires before $[\sigma_H]$ builds, blocking the cascade. Sweep~C (25 conditions, 50 replicates) directly replicates the Fujita protocol and reproduces the $52\%$ vs.\ $\approx 5\%$ contrast; no arc parameter was optimized to match this ratio---all $\Gamma = (K, n, \varepsilon)$ values are drawn from the BRENDA database and published kinetics. This mechanistic prediction is inexpressible in classical Bio-PN lacking $\Gamma$-threshold semantics.
...
An important clarification on scope: rate functions $\Phi$ are not a new feature of SHPN --- they are inherited unchanged from standard Bio-PN, where kinetic expressions (Michaelis-Menten, Hill, mass-action) routinely reference any place by name. A natural question is whether $\Phi$, being able to encode any function of any state variable, makes $F_s$ redundant. It does not: $\Phi$ and $F_s$ are tools for two structurally different things. $\Phi$ operates over $G_E$ --- it models continuous, reversible kinetics across the full state space, and its dependencies may be cyclic. $F_s$ operates over $G_S$ --- it models discrete, irreversible commitment boundaries whose topology must be acyclic. The structural theorems (Acyclicity, Preemption, Irreversible Basin Partition) are properties of $G_S$, not of $\Phi$: they cannot be recovered by encoding the dynamics in rate functions alone, because $\Phi$ functions are not subject to the DAG constraint that makes those theorems provable. Put differently, $\Phi$ gives transitions access to the genome's full epigenetic state space (sensing any concentration at any time); $F_s$ imposes the topological commitment structure that makes some of those states unreachable once a threshold is crossed.

To see why $\Phi$ alone cannot replace $F_s$, consider the limit: a Petri net with all regulation encoded in $\Phi$ and no arcs at all. Such a net has no token flow---transitions fire but consume and produce nothing, so the marking $M$ never changes from $M_0$. $\Phi$ reads the same static values forever; there is no dynamics, no state transition, no basin boundary. This is equivalent to a system of rate equations evaluated at fixed initial conditions: trivially solvable but biologically inert. The moment you add a normal arc you get stoichiometric dynamics; the moment you add a signal flow arc you get an irreversible commitment boundary with computable threshold $M_{\text{commit}} = \theta + W_s$. Token \textit{consumption}---not rate-function reading---is the mechanism that creates the one-way barrier. $\Phi$ can express what a transition \textit{reads}; only $F_s$ arcs determine what it \textit{destroys}, and it is destruction that makes commitment irreversible.
...
The three-phase commitment semantics are directly observable in these trajectories (Figure~\ref{fig:preemption_cascade}).
...
\begin{itemize}
\item \textbf{Layer 0 (starvation sensing):} KinA$_{\text{kinase}}$ designated as signal; produced only under nutrient deprivation; drives the phosphorelay entry transition.
\item \textbf{Layer 1 (phosphorelay integration):} Spo0A$\sim$P designated as signal; accumulates through the four-protein relay (KinA $\to$ KinA$_P$ $\to$ Spo0F$_P$ $\to$ Spo0A$\sim$P) via stochastic step-by-step phosphotransfer.
\item \textbf{Layer 2 (commitment gate):} $\sigma_H$ designated as signal; Hill-4 positive-feedback switch; gates $T_{\text{septation}}$ via $[\sigma_H]^2$; commitment separatrix at $\theta_{\sigma H} = 1.60\,\mu$M.
\item \textbf{Layer 3 (execution programme):} $\sigma_F$, $\sigma_E$, $\sigma_G$, $\sigma_K$ designated as signals; continuous sigma cascade post-septation; quasi-deterministic once PreemptionCheck unlocks.
\end{itemize}

Total: 40 places, 36 transitions, 114 arcs, 4 signal layers (Figure~\ref{fig:model_topology}).
...
\begin{itemize}
\item $\pm 20\%$ phosphorelay rate variation: natural fraction $[42\%, 62\%]$; abrupt fraction $< 10\%$
\item $\pm 20\%$ SinR/SinI gate variation: natural fraction $[40\%, 64\%]$; abrupt fraction $[0\%, 8\%]$
\item $\pm 20\%$ $\sigma_H$ feedback variation: natural fraction $[44\%, 60\%]$; abrupt fraction $[0\%, 8\%]$
\end{itemize}
...
pre-print~\cite{simao2025unifying} (arXiv:2601.00036, submitted December~2025).
...
under the mature definition before formal journal submission. The GATA1/PU.1
haematopoietic lineage commitment model is in active preparation~\cite{gata2026}. These developmental works are cited to establish breadth of applicability and to
avoid duplication should readers encounter the pre-prints independently.
...
The \textit{Bacillus subtilis} sporulation model demonstrates the formalism's analytical capability through basin boundary computation to determine commitment thresholds from signal hierarchical structure. Predecessor analyses applying earlier versions of the formalism to other biological systems are cited in Section~\ref{sec:broader} as pre-print references; the B.\ subtilis case is presented here as the sole fully validated application.

\vspace{3pt}
\noindent The analysis proceeds through hierarchical layer assignment establishing four-layer architecture from starvation sensing (KinA$_{\text{kinase}}$, Layer~0) through phosphorelay integration (Spo0A$\sim$P, Layer~1) and commitment gate ($\sigma_H$, Layer~2) to execution machinery ($\sigma_F$--$\sigma_K$, Layer~3). Signal consumption semantics enable modeling irreversible commitment through $\Gamma$-threshold gating, capabilities inexpressible with classical test arcs. Biological validation reproduces the Fujita and Losick~\cite{fujita2005} experimental observation: stochastic sweep simulation (Sweep~C, 25 conditions, 50 replicates each) achieves $48$--$52\%$ natural sporulation and $\approx 5\%$ abrupt-pulse sporulation, matching Fujita Figs.~2A/B and 2C/D respectively, with no arc parameter optimized to match these fractions.
...
\textbf{Hierarchical signal integration:} Multi-layer architecture enables sophisticated decision logic regardless of designated signal identity. For \textit{B. subtilis} (v9 model): Layer~0 (starvation sensing) designates KinA$_{\text{kinase}}$ as signal---active only under nutrient deprivation. Layer~1 (phosphorelay integration) designates Spo0A$\sim$P, accumulating through the four-protein relay (KinA~$\to$~Spo0F~$\to$~Spo0A). Layer~2 (commitment gate) designates $\sigma_H$, whose Hill-4 positive-feedback switch crosses $\theta_{\sigma H} = 1.60\,\mu$M stochastically. Layer~3 (execution) designates $\sigma_F$, unlocking the full sigma cascade post-septation. This stratification enables robust decisions: single sensor failure does not prevent commitment (redundancy), but designated signal collapse does (preemption). Hierarchy balances reliability (multiple pathways) with safety (designated resource gating). The architecture generalizes: calcium signaling designates Ca$^{2+}$, chemotaxis designates cAMP, transcription quality control designates GTP---same hierarchical organization, different signal identities.
...
\item \textbf{Unified framework} enables topology-driven threshold computation: $M_{\text{commit}} = \theta + W_s$ computable from literature parameters without simulation or fitting, reducing EXPSPACE-complete reachability to $O(1)$ arithmetic.
\end{enumerate}
