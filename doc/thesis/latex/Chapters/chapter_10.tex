\hypertarget{chapter-10-intelligent-parameter-inference-from-brenda}{%
\chapter{Chapter 10: Intelligent Parameter Inference from
BRENDA}\label{chapter-10-intelligent-parameter-inference-from-brenda}}

\hypertarget{introduction}{%
\section{10.1 Introduction}\label{introduction}}

\textbf{Kinetic parameters} (Vmax, Km, kcat, Ki) determine reaction
\textbf{rates and regulation} in Bio-PN models. \textbf{Manually finding
parameters} for dozens of enzymes is: 1. \textbf{Time-consuming}:
Literature search takes hours per enzyme 2. \textbf{Inconsistent}:
Different studies report different values (organism, pH, temperature) 3.
\textbf{Incomplete}: Many enzymes lack published parameters

\textbf{This chapter presents automatic parameter inference} from
\textbf{BRENDA} (BRaunschweig ENzyme DAtabase), the largest enzyme
kinetics repository.

\textbf{BRENDA database} (www.brenda-enzymes.org): - \textbf{83,000+
enzymes} classified by EC number - \textbf{2.7 million} kinetic
parameters (Km, kcat, Ki, optimal pH, etc.) - \textbf{Organism-specific
data}: Yeast, human, bacteria - \textbf{SOAP API}: Programmatic access
(requires free registration)

\textbf{SHYpn parameter inference}: 1. \textbf{EC number lookup}: User
specifies enzyme (e.g., EC 2.7.1.1 = hexokinase) 2. \textbf{Organism
filtering}: Prioritize yeast (\emph{S. cerevisiae}), then human, then
all 3. \textbf{Statistical aggregation}: Compute median Km (robust to
outliers) 4. \textbf{Context-aware heuristics}: Adjust by substrate, pH,
temperature 5. \textbf{Local caching}: Build SQLite database of queried
parameters (offline use)

\textbf{Benefits}: - \textbf{Speed}: Fetch all 10 glycolysis enzyme
parameters in \textless10 seconds - \textbf{Accuracy}: Median of 50+
literature values (robust estimate) - \textbf{Reproducibility}: Cached
parameters ensure consistency across sessions

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{brenda-data-structure}{%
\section{10.2 BRENDA Data Structure}\label{brenda-data-structure}}

\hypertarget{enzyme-commission-ec-numbers}{%
\subsection{10.2.1 Enzyme Commission (EC)
Numbers}\label{enzyme-commission-ec-numbers}}

\textbf{EC classification} (hierarchical 4-level code): - \textbf{EC
2.x.x.x}: Transferases (transfer functional groups) - \textbf{EC
2.7.x.x}: Transferring phosphorus-containing groups - \textbf{EC
2.7.1.x}: Phosphotransferases with alcohol group as acceptor -
\textbf{EC 2.7.1.1}: Hexokinase (ATP:D-hexose 6-phosphotransferase)

\textbf{Examples}: - EC 2.7.1.1: Hexokinase (glucose + ATP \textrightarrow G6P + ADP)
- EC 2.7.1.11: 6-Phosphofructokinase (F6P + ATP \textrightarrow F-1,6-BP + ADP) - EC
1.2.1.12: Glyceraldehyde-3-phosphate dehydrogenase (GAPDH)

\textbf{SHYpn usage}: Transition property panel has ``EC Number'' field
\textrightarrow fetches BRENDA data

\hypertarget{kinetic-parameters-in-brenda}{%
\subsection{10.2.2 Kinetic Parameters in
BRENDA}\label{kinetic-parameters-in-brenda}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Km} & Michaelis constant (substrate affinity) & mM, μM &
Km(glucose) = 0.1 mM \\
\textbf{kcat} & Turnover number (max reactions per enzyme per second) &
1/s & kcat = 100 s⁻¹ \\
\textbf{Ki} & Inhibition constant (inhibitor affinity) & mM, μM &
Ki(ATP) = 5.0 mM \\
\textbf{Vmax} & Maximum reaction rate (derived: Vmax = kcat · {[}E{]}₀)
& mM/s & Vmax = 1.0 mM/s \\
\end{longtable}

\textbf{BRENDA data structure} (per parameter entry):
...
\textbf{Challenges}: 1. \textbf{Heterogeneity}: Values vary by organism,
substrate, conditions 2. \textbf{Outliers}: Some entries are erroneous
(typos, unit errors) 3. \textbf{Incompleteness}: Not all enzymes have Km
for all substrates

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{brenda-soap-api}{%
\section{10.3 BRENDA SOAP API}\label{brenda-soap-api}}

\hypertarget{authentication}{%
\subsection{10.3.1 Authentication}\label{authentication}}

\textbf{BRENDA API requires free credentials}: 1. Register at
https://www.brenda-enzymes.org/ 2. Receive username and password via
email 3. Store in SHYpn config file
(\passthrough{\lstinline!\~/.config/shypn/brenda\_credentials.json!})
...
\hypertarget{soap-client}{%
\subsection{10.3.2 SOAP Client}\label{soap-client}}

\textbf{SOAP (Simple Object Access Protocol)}: XML-based web service
protocol

\textbf{Python implementation} (using \passthrough{\lstinline!zeep!}
library):
...
class BRENDAConnector:
    ...
    def _parse_numeric_value(self, value_str: str) -> Optional[float]:
        ...
        Handles:
        - Simple numbers: "0.12" \textrightarrow 0.12
        - Ranges: "0.1-0.3" \textrightarrow 0.2 (midpoint)
        - Scientific notation: "1.2e-3" \textrightarrow 0.0012
        - Inequalities: "<0.5" \textrightarrow 0.5 (upper bound)
        """
        import re
...
\hypertarget{example-fetching-hexokinase-km}{%
\subsection{10.3.3 Example: Fetching Hexokinase
Km}\label{example-fetching-hexokinase-km}}
...
\hypertarget{statistical-aggregation}{%
\section{10.4 Statistical Aggregation}\label{statistical-aggregation}}

\hypertarget{median-as-robust-estimator}{%
\subsection{10.4.1 Median as Robust
Estimator}\label{median-as-robust-estimator}}

\textbf{Problem}: BRENDA values have \textbf{high variance} (10×
difference between min/max)

\textbf{Causes}: 1. \textbf{Experimental error}: Different labs, methods
2. \textbf{Biological variation}: Different strains, growth conditions
3. \textbf{Data entry errors}: Typos, unit confusion (μM vs.~mM)

\textbf{Solution}: Use \textbf{median} (50th percentile) instead of mean
- \textbf{Median is robust to outliers} (1 extreme value doesn't skew
result) - \textbf{Mean is sensitive} (one 100× outlier pulls average up)

\textbf{Example} (hexokinase Km for glucose in yeast):
...
\subsection{10.4.2 Confidence Intervals}\label{confidence-intervals}}

\textbf{95\% confidence interval}: Range containing true value with 95\%
probability

\textbf{Bootstrap method} (non-parametric): 1. Resample data with
replacement (1000 times) 2. Compute median of each resample 3. Take
...
\hypertarget{context-aware-heuristics}{%
\section{10.5 Context-Aware Heuristics}\label{context-aware-heuristics}}

\hypertarget{organism-priority}{%
\subsection{10.5.1 Organism Priority}\label{organism-priority}}

\textbf{Challenge}: BRENDA contains data from \textbf{5000+ organisms},
but user cares about specific organism (e.g., yeast)

\textbf{SHYpn strategy}: \textbf{Hierarchical filtering}: 1.
\textbf{Primary}: User-specified organism (\emph{S. cerevisiae}) 2.
\textbf{Secondary}: Related organisms (other fungi) 3.
\textbf{Tertiary}: All organisms (if \textless5 primary values)
...
\subsection{10.5.2 Quality Filtering}\label{quality-filtering}}

\textbf{Some BRENDA entries are low-quality}: - No literature citation -
Vague commentary (``approximate value'') - Extreme outliers (100×
different from median)
...
\hypertarget{local-database-caching}{%
\section{10.6 Local Database Caching}\label{local-database-caching}}

\hypertarget{motivation}{%
\subsection{10.6.1 Motivation}\label{motivation}}

\textbf{Problem}: BRENDA API is slow (500 ms - 2 seconds per query)

\textbf{Solution}: Cache results in local SQLite database - First query:
Fetch from BRENDA + save to DB (2 seconds) - Subsequent queries: Read
from DB (10 ms) \textrightarrow \textbf{200× faster}

\hypertarget{database-schema}{%
\subsection{10.6.2 Database Schema}\label{database-schema}}

\textbf{File}:
\passthrough{\lstinline!\~/.config/shypn/brenda\_cache.db!}
...
\textbf{Design}: - \passthrough{\lstinline!brenda\_raw\_data!}: Stores
every individual measurement (immutable) -
\passthrough{\lstinline!brenda\_statistics!}: Cached aggregated
statistics (recomputed on demand)

\hypertarget{implementation}{%
\subsection{10.6.3 Implementation}\label{implementation}}
...
\hypertarget{workflow-with-caching}{%
\subsection{10.6.4 Workflow with Caching}\label{workflow-with-caching}}

\textbf{Integrated fetcher}:
...
class CachedParameterInferencer:
    ...
    def infer_km(self, ec_number: str, substrate: str) -> Dict[str, float]:
        ...
        2. If found and fresh \textrightarrow return immediately
        3. Else: Fetch from BRENDA API \textrightarrow save to DB \textrightarrow return
        """
        # Try cache first
...
\hypertarget{ui-integration}{%
\section{10.7 UI Integration}\label{ui-integration}}

\hypertarget{fetch-from-brenda-button}{%
\subsection{10.7.1 ``Fetch from BRENDA''
Button}\label{fetch-from-brenda-button}}

\textbf{Transition property panel} includes: 1. \textbf{EC Number
field}: User enters EC number (e.g., ``2.7.1.1'') 2. \textbf{``Fetch
from BRENDA'' button}: Triggers parameter inference 3. \textbf{Results
table}: Shows median Km, Vmax, Ki with confidence intervals 4.
\textbf{``Apply'' button}: Fills rate function parameters

\textbf{User workflow}: 1. User creates transition (e.g.,
``Hexokinase'') 2. User enters EC number ``2.7.1.1'' 3. User clicks
``Fetch from BRENDA'' 4. SHYpn displays:
\passthrough{\lstinline!Km(glucose):  0.165 mM [0.10-0.22]  (23 values)    kcat:         100 s⁻¹ [80-120]      (15 values)!}
5. User clicks ``Apply'' \textrightarrow Rate function auto-configured

\hypertarget{batch-parameter-inference}{%
\subsection{10.7.2 Batch Parameter
Inference}\label{batch-parameter-inference}}

\textbf{For entire pathways} (e.g., glycolysis with 10 transitions): 1.
User selects \textbf{``Enrich All Transitions''} menu item 2. SHYpn
iterates over all transitions with EC numbers 3. For each: Fetch BRENDA
parameters \textrightarrow Update rate function 4. Progress bar shows ``5/10
...
\section{10.8 Limitations and Future
Work}\label{limitations-and-future-work}}

\hypertarget{current-limitations}{%
\subsection{10.8.1 Current Limitations}\label{current-limitations}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Missing data}: Not all enzymes in KEGG are in BRENDA

  \begin{itemize}
  \tightlist
  \item
    \textbf{Mitigation}: Fallback to literature search or user input
  \end{itemize}
\item
  \textbf{Vmax vs.~kcat}: BRENDA provides kcat, but Vmax = kcat ·
  {[}E{]}₀ requires enzyme concentration

  \begin{itemize}
  \tightlist
  \item
    \textbf{Solution}: Assume default {[}E{]}₀ = 0.01 mM (adjustable by
    user)
  \end{itemize}
\item
  \textbf{Multi-substrate kinetics}: Most BRENDA data is for
  single-substrate reactions

  \begin{itemize}
  \tightlist
  \item
    \textbf{Limitation}: Ordered bi-bi kinetics not well-covered
  \end{itemize}
\item
  \textbf{pH/temperature dependence}: BRENDA values at different
  conditions

  \begin{itemize}
  \tightlist
  \item
    \textbf{Future}: Temperature correction using Arrhenius equation
  \end{itemize}
\end{enumerate}

\hypertarget{future-enhancements}{%
\subsection{10.8.2 Future Enhancements}\label{future-enhancements}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Machine learning refinement}:
  ...
  \item
    Predict Km for enzymes without BRENDA data (based on substrate
    structure)
  \end{itemize}
\item
  \textbf{SABIO-RK integration}:
...
\hypertarget{summary}{%
\section{10.9 Summary}\label{summary}}

\textbf{Chapter 10 presented intelligent parameter inference from
BRENDA}:
...
\tightlist
\item
  \textbf{BRENDA SOAP API}: Fetch Km, kcat, Ki from 2.7 million
  measurements
\item
  \textbf{Statistical aggregation}: Median + 95\% CI (robust to
  outliers)
\item
  \textbf{Context-aware heuristics}:

  \begin{itemize}
  \tightlist
  \item
    Organism priority (yeast \textgreater{} human \textgreater{} all)
  \item
  ...
  \end{itemize}
\item
  \textbf{Local caching}: SQLite database \textrightarrow 200× faster on repeated
  queries
\item
  \textbf{UI integration}: ``Fetch from BRENDA'' button, batch
  enrichment
\item
  \textbf{Performance}: Entire glycolysis pathway (10 enzymes) enriched
  in \textless10 seconds
\end{enumerate}

\textbf{Key innovation}: \textbf{Progressive data accumulation} - Each
BRENDA query adds to local database - Over time, user builds
comprehensive organism-specific kinetics library - Enables offline
modeling (no internet required after initial fetch)

\textbf{Example}: User models yeast metabolism for 1 month \textrightarrow accumulates
500 EC numbers × 50 values each = 25,000 cached measurements \textrightarrow all
future yeast models use cached data (instant parameter inference)

\textbf{Next chapter} (Chapter 11): Hybrid simulation engine (ODE +
Gillespie + Timed + Burst).
