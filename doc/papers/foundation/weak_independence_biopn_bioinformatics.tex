...
{\small\hyphenpenalty=50\exhyphenpenalty=50
...
\end{equation}
where: $P$ (places/\hspace{0pt}chemical species); $T$ (tran\-si\-tions/\hspace{0pt}re\-ac\-tions); $F \subseteq (P \times T) \cup (T \times P)$ (flow re\-la\-tion); $W: F \to \mathbb{R}^+$ (arc weights); $M_0: P \to \mathbb{N}_0 \cup \mathbb{R}_0^+$ (ini\-tial mark\-ing); $K: P \to \mathbb{N} \cup \{\infty\}$ (ca\-pac\-i\-ty); $\Phi: T \to (\mathbb{R}^n \to \mathbb{R})$ (rate func\-tions); $\Sigma \subseteq (P \times T) \setminus F$ (reg\-u\-la\-to\-ry arcs); \\
{\footnotesize $\Theta: T \to \{\text{Internal}, \text{Source}, \text{Sink}, \text{Exchange}\}$ (environment);} \\
{\footnotesize $\Delta: T \times T \to \{\text{Independent}, \text{Competitive},\linebreak[0] \text{Convergent}, \text{Regulatory}\}$ (dependency);} \\
{\footnotesize $\tau: T \to \{\text{Continuous}, \text{Stochastic}, \text{Timed}, \text{Immediate}\}$ (transition type);} \\
$\rho: P \to \text{Formula}$ (bio\-chem\-i\-cal for\-mu\-las).
}
...
Detects rate formula-based regulation (without structural arcs, correct for Bio-PNs). For each transition $t$: (1) parse $\Phi(t)$ to extract variable names $V$, (2) compute $\RegSet{t} = V \setminus (\preset{t} \cup \postset{t})$, (3) classify as catalyst (unchanged), activator (positive coefficient), or inhibitor (negative coefficient).
