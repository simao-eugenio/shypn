...
% line numbers
% line numbers removed for clean copy
...
Cells make irreversible commitment decisions based on metabolite availability. When bacteria enter sporulation, cancer cells commit to division, or neurons release neurotransmitters, these transitions occur at specific molecular thresholds. Yet classical formalisms cannot derive these thresholds as structural properties of network topology---they require global parameter fitting or numerical simulation, so the threshold is an output of data, not of network structure.
...
$(t_i,p_j) \in F_s$.
The SignalEnabled predicate for $t_i$ requires
$M(p_i) \geq \theta(t_i) + W_s((p_i,t_i))$.
If $M(p_i) < \theta(t_i)$, then since $W_s((p_i,t_i)) > 0$,
...
$\neg\text{PreemptionCheck}(t_j,M)$ for all $t_j \in p_j^\bullet$.
...
$\lambda(p_m)-\lambda(p_i) < k$: if $M(p_i) < \theta(t_i)$, then
$\neg\text{SignalEnabled}(t_m,M)$ for all $t_m$ producing $p_m$ via $F_s$.
Therefore $\neg\text{PreemptionCheck}(t_j,M)$ for all $t_j \in p_m^\bullet$
(direct consequence of $\neg\text{SignalEnabled}$ of $p_m$'s producers).
By inductive hypothesis applied to $(p_m,p_j)$: this propagates
$\neg\text{SignalEnabled}$ to all $F_s$-producers of $p_j$, hence
$\neg\text{PreemptionCheck}(t_j,M)$ for all $t_j \in p_j^\bullet$.
Transitive closure over the $F_s$-acyclic path completes the proof.
\end{proof}
...
\textbf{Test arcs} ($F_t$): Model catalysis through non-consuming local observation. Require $M(p) > \tau_t((p,t))$ without token transfer, enabling rate acceleration via arc-connected catalysts.
Test arcs are the appropriate model for catalysis on two independent grounds.
First, they are the established formalism for read-only access in Petri nets (contextual/read arcs, \cite{baldan2001}): they are semantically distinct from loops ($W(p,t)=W(t,p)$), which consume and regenerate the same quantity of tokens, because loops contribute to the stoichiometric firing rule while test arcs leave the marking unchanged --- a semantic difference independent of concurrency.
...
Example & GTP\_pool $\xrightarrow{F_s}$ $T_{\text{septation}}$ & $\sigma_H$ gates $T_{\text{septation}}$ via $\Phi(\sigma_H^2)$ \\
\bottomrule
\end{tabular}
\end{table}
...
This follows the two-layer architecture of stochastic Petri nets~\cite{goss1998}: the
\emph{structural enabling} layer (Enabled predicate, topology-based) determines
\emph{whether} a transition may fire; the \emph{kinetic propensity} layer ($\Phi$)
determines \emph{how fast} it fires.
...
\text{TestEnabled}(t,M) &\equiv \forall p \in \text{Test}(t): M(p) > \tau_t((p,t)) \\
...
\begin{enumerate}
\item \textbf{Pre-commitment} (before firing): $M(p_s) < M_{\text{commit}}(p_s)$ --- signal not yet at threshold, transition disabled, cell uncommitted
\item \textbf{Commitment boundary}: $M(p_s) = M_{\text{commit}}(p_s)$ --- transition enabled; firing creates irreversibility
\item \textbf{Post-commitment} (after firing): $M'(p_s) = \theta(t) < M_{\text{commit}}(p_s)$ --- signal tokens consumed, residual marking prevents return; cell committed
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{%
        ...
        ($N_0 = 1440\,\mu$M, SinR $= 12\,\mu$M, run\_20260719\_170034).
        ...
        The $\sigma_H$ effective threshold ($\theta_{\sigma H} = 1.60\,\mu$M, identified via simulation) separates sporulating from vegetative replicates: once crossed, GTP tokens are consumed irreversibly via the signal-flow arc and all downstream layers fire in lock-step.%
    }
...
At $t_\mathrm{commit} = 300\,\mathrm{min}$ the stochastic $\sigma_H$ ramp crosses $\theta_{\sigma H}$ in a subset of replicates: these enter the commitment boundary and fire $T_{\mathrm{septation}}$, consuming GTP tokens via the signal-flow arc irreversibly.
...
\item $\Phi: T \rightarrow \text{RateFunctions}$ assigns kinetic rate laws to transitions.
  ...
  Following the standard two-layer architecture of stochastic Petri nets~\cite{goss1998},
  $\Phi$ does not appear in the structural Enabled predicate: enabling is topological,
  propensity is kinetic.
\item $C: P \rightarrow \text{Compartments}$ assigns each place to a spatial
  compartment (default: cytoplasm), used by the simulation engine for
  volume-based concentration conversion ($c = N / (N_A \times V_C)$) and
  multi-compartment diffusion.
  $C$ does not appear in the structural Enabled predicate or Firing rule:
  compartment assignment is a simulation-level parameter, not a topological one.
  % [NOTE for revision 3: legacy description was C: F → Stoich (stoichiometric
  %  coefficients for ODE arcs). Corrected to match source implementation where
  %  C: P → Compartments. The ODE stoichiometry is handled by W (arc weights).]
\item $F_t \subseteq P \times T$ defines test arcs (catalysis, non-consuming); test arc
  enablement requires $M(p) > \tau_t((p,t))$ with default $\tau_t = 0$ (presence
  check $M(p) > 0$) --- the sensing threshold is uniformly~0 unless explicitly set
...
\begin{proof}
General reachability in Petri nets is Ackermann-complete~\cite{czerwinski2021} (and hence not primitive-recursive), with state space that grows non-primitively-recursively in the number of places. For flat model with $\sum_i n_i$ places distributed across $k$ layers, reachability graph has $O(\prod_i C^{n_i})$ states where $C$ is token bound.
...
\vspace{3pt}
\noindent\textbf{Vertical information broadcast.} The same places, when we assign type ``signal,'' broadcast availability to higher regulatory layers via signal flow arcs $F_s$. For example, GTP\_pool gating septation: $(\text{GTP\_pool},\, T_{\text{septation}}) \in F_s$ with $W_s > 0$ (structural commitment arc consumed on firing). Spo0A$\sim$P broadcasting phosphorelay output: $(\text{Spo0A}{\sim}\text{P},\, t_{\sigma H}) \in F_s$ (integration-to-commitment gating). Note that $\sigma_H$ gates $T_{\text{septation}}$ kinetically via rate function $\Phi(T_{\text{septation}}) \propto [\sigma_H]^2$ (remote sensing), not via a signal flow arc---the commitment separatrix $\theta_{\sigma H} = 1.60\,\mu$M is a kinetic threshold identified by simulation, not a structural $M_{\text{commit}} = \theta + W_s$ value.
...
\vspace{3pt}
\noindent\textbf{Layer 2 (Commitment Gate):} $\sigma_H$ designated as signal ($\lambda(\sigma_H) = 2$). Produced by a Hill-4 positive-feedback switch on Spo0A$\sim$P ($T_{\sigma H}$: $\Phi \propto \text{Spo0A}{\sim}\text{P}^4 / (50 + \text{Spo0A}{\sim}\text{P}^4)$); gates $T_{\text{septation}}$ kinetically via remote sensing ($\Phi \propto \sigma_H^2$). The $\sigma_H$ effective threshold $\theta_{\sigma H} = 1.60\,\mu$M is identified via simulation (Sweep~A): only replicates crossing it fire $T_{\text{septation}}$, which consumes GTP tokens via the structural signal-flow arc, unlocking Layer~3.
...
No simulation, no reachability analysis, no numerical integration---direct arithmetic from model specification. This contrasts with the Ackermann-complete reachability analysis~\cite{czerwinski2021} required for general threshold determination in unstructured Petri nets.
\end{proof}

This establishes topology-driven predictability: the threshold is \textit{structurally localized} --- once the signal topology is fixed, it is determined by at most two measurable quantities per signal arc ($\theta$ and $W_s$), extracted from biological literature (BRENDA database, biochemistry textbooks). The claim is not that the threshold is parameter-free, but that it does not require fitting a global nonlinear model: extract parameters from biological literature, instantiate formalism functions $\theta$ and $W_s$, compute threshold via closed-form formula, obtain quantitative prediction without simulation or parameter optimization. For the \textit{B.~subtilis} sporulation model this yields the $\sigma_H$ effective commitment threshold, identified via simulation in Proposition~\ref{prop:sigmaH_separatrix} in Section~\ref{sec:validation}.
...
\begin{figure}[p]
    \centering
    {\large\textbf{Signal Hierarchical Petri Net: \textit{B.~subtilis} Sporulation (v9)}}
    \par\vspace{6pt}
    \caption{%
...
\item $\sigma_H$ effective threshold $\theta_{\sigma H} \approx 1.60\,\mu$M: identified empirically via Sweep~A bifurcation (run\_20260611\_231304); this is a kinetic threshold from $\Phi(T_{\text{septation}}) \propto [\sigma_H]^2$, not a structural $M_{\text{commit}} = \theta + W_s$ computation
\item SinR/SinI gate: T32 $K_m \approx 52.9\,\mu$M calibrated to match the stochastic bimodal gate dynamics (log-log interpolation across Sweep~C, run\_20260719\_170034); calibration target was gate shape, not the 52\% fraction
\end{itemize}
...
\subsubsection{Sweep~C: Fujita Perturbation Protocol (run\_20260719\_170034)}
...
\begin{figure}[htbp]
    ...
    \caption{%
        ...
        White line: mean sporulation order parameter $\varphi(t)$ averaged over 50 stochastic $\tau$-leaping replicates (run\_20260719\_170034, $N_0 = 1440\,\mu$M, 6-h horizon).
...
\begin{corollary}[Basin Boundary Characterization --- \textit{B.~subtilis}]
For the \textit{B.~subtilis} sporulation model, the reachability space $\mathcal{R}(M_0)$ partitions into two regions. Using the notation of Theorem (Irreversible Basin Partition) with the SinR/$\sigma_H$ commitment gate:
\begin{align*}
B_{\text{below}} &= \{M \in \mathcal{R}(M_0) : M(\text{GTP\_pool}) < M_{\text{commit}}(\text{GTP\_pool})\} \\
B_{\text{boundary}} &= \{M \in \mathcal{R}(M_0) : M(\text{GTP\_pool}) = M_{\text{commit}}(\text{GTP\_pool})\} \\
B_{\text{above}} &= \{M \in \mathcal{R}(M_0) : M(\text{GTP\_pool}) > M_{\text{commit}}(\text{GTP\_pool}) \wedge T_{\text{septation}}\text{ has fired}\}
\end{align*}
where GTP\_pool is the signal-flow arc input to $T_{\text{septation}}$ and $M_{\text{commit}}(\text{GTP\_pool}) = \theta(T_{\text{septation}}) + W_s((\text{GTP\_pool}, T_{\text{septation}}))$.
The SinR inhibitor arc (threshold $7.5\,\mu$M) and the $[\sigma_H]^2$ kinetic factor constitute the complementary structural and kinetic commitment barriers. No firing sequence from $B_{\text{below}}$ reaches $B_{\text{above}}$ after $T_{\text{septation}}$ fires.
\end{corollary}

\begin{proof}
Direct application of Theorem (Irreversible Basin Partition) with GTP\_pool as the signal-flow arc input to $T_{\text{septation}}$.
When the SinR/SinI gate opens and $[\sigma_H]$ is above the effective kinetic threshold, $T_{\text{septation}}$ fires from $B_{\text{boundary}}$, consuming GTP tokens via the signal-flow arc and producing Septum via the curved signal-flow arc. The resulting Septum inhibitor arc ($\text{Septum} \to T_{\text{septation}}$, curved inhibitor) then blocks re-firing, making the partition absorbing. With no $F_s$ arc regenerating GTP\_pool after consumption, $B_{\text{below}}$ is absorbing per the theorem.
\end{proof}
...
\noindent\textit{Verification (empirical, not proof):} This robustness bound was verified by varying each parameter class by $\pm20\%$ in sweep simulations.
\begin{itemize}
\item \textbf{Phosphorelay kinetics ($\pm20\%$):} natural sporulation $42\text{--}62\%$; abrupt $< 10\%$.
\item \textbf{SinR/SinI gate ($\pm20\%$):} natural $40\text{--}64\%$; gate remains functional.
\item \textbf{$\sigma_H$ feedback ($\pm20\%$):} natural $44\text{--}60\%$; commitment timing shifts $\pm 25\,$min.
\end{itemize}
The qualitative Fujita asymmetry is preserved across all perturbations without refitting.
...
Stochastic sweep simulations use a $\tau$-leaping engine; deterministic simulations use an ODE solver with Hill-function kinetics. Both engines are implemented in the SHPN simulator (source code: \url{https://github.com/simao-eugenio/shypn}, DOI:~\href{https://doi.org/10.5281/zenodo.21478129}{10.5281/zenodo.21478129}). All signal arc parameters ($\Gamma = (K, n, \varepsilon)$, $\theta$, $W_s$) are drawn from published sources (BRENDA, Voet \& Voet~\cite{voet2016}); no parameter was adjusted to match the Fujita efficiency fractions. Complete model specifications, rate constants, initial conditions, and sensitivity analysis are documented in~\cite{subtilis2026}.
...
\item \textbf{Unified framework} enables topology-driven threshold computation: $M_{\text{commit}} = \theta + W_s$ computable from literature parameters without simulation or fitting, reducing Ackermann-complete reachability~\cite{czerwinski2021} to $O(1)$ arithmetic.
\end{enumerate}
...
% Funding, Author Contributions, Competing Interests, and Data Availability
% statements have been removed from the manuscript body per PLOS ONE requirements
% (Items 3-6 of the technical amendments email, 2026-07-26).
% These statements are entered in the online submission system.
%
% Data Availability: Source code at https://github.com/simao-eugenio/shypn
% (MIT license) and archived on Zenodo DOI: 10.5281/zenodo.21478129 (v2.7.1).
% Sweep results: run_20260719_170034 (corrected bacillus_sporulation_v9.shy model).
