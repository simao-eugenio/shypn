...
%DIF ADD revision_2/main_plos_one_revision2.tex   Mon Jul 20 12:38:59 2026
...
% Introduction
% [R1-m1] Add dual-graph schematic figure (G_E mass graph + G_S control POSet)
...
\end{tikzpicture}
\caption{\textbf{Dual-graph architecture of SHPN.} The execution graph $G_E$
...
): they are semantically distinct from loops ($W(p,t)=W(t,p)$), which consume and regenerate the same quantity of tokens, because loops contribute to the stoichiometric firing rule while test arcs leave the marking unchanged.
Second, test arcs have zero mass transfer ($\Delta M = 0$), hence zero entropy production rate (EPR) contribution in stochastic thermodynamics; loops have non-zero mass flow and non-zero EPR. This thermodynamic distinction is essential for the formalism's physical interpretation: catalysts that enable reactions without being consumed must not contribute to the dissipation budget of the commitment decision.
}\DIFaddend 
...
% or designated signal places), why the structural constraint is on G_S topology
...
and enters the entropy production rate computation through the propensity
ratio $\Phi^+(M)/\Phi^-(M)$ used in stochastic thermodynamics.
}\DIFaddend A natural question is whether $\Phi$, being able to encode any function of any state variable, makes $F_s$ redundant. It does not: $\Phi$ operates over $G_E$---modelling continuous, reversible kinetics whose read dependencies may be cyclic---while $F_s$ operates over $G_S$---modelling discrete, irreversible commitment whose topology must be acyclic. The structural theorems (Acyclicity, Preemption, Basin Partition) are properties of $G_S$: they cannot be recovered from rate functions alone because $\Phi$ is not subject to the DAG constraint that makes them provable. In the limit---a net with all regulation in $\Phi$ and no arcs---the marking never changes from $M_0$ and there is no basin boundary. The moment you add a signal flow arc you get an irreversible commitment boundary with threshold $M_{\text{commit}} = \theta + W_s$. $\Phi$ expresses what a transition \textit{reads}; only $F_s$ arcs determine what it \textit{destroys}, and it is destruction that makes commitment irreversible.
...
\item $\Phi: T \rightarrow \text{RateFunctions}$ assigns kinetic rate laws to transitions\DIFaddbegin \DIFadd{.
  ...
  and the entropy production rate via the propensity ratio $\Phi^+(M)/\Phi^-(M)$.
...
%DIFDELCMD < %%%
\textbf{\DIFdel{Case 3: $\sigma_H$ feedback strength.}} %DIFAUXCMD
\DIFdel{Varying k\_syn\_factor by $\pm 20\%$ shifts the time at which $[\sigma_H]$ crosses $\theta_{\sigma H}$ by $\pm 25\,\text{min}$, not the separatrix value itself.
Natural fraction varies $44\text{--}60\%$ (stochastic window shifts slightly).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Union of all cases: natural sporulation $\in [42\%, 62\%]$; abrupt $\in [0\%, 10\%]$.
}\DIFdelend \DIFaddbegin \DIFadd{.
}\item \textbf{\DIFadd{$\sigma_H$ feedback ($\pm20\%$):}} \DIFadd{natural $44\text{--}60\%$; commitment timing shifts $\pm 25\,$min.
}\end{itemize}
...
% [R1-C3] Acyclicity of G_S vs cycles in G_E -- explicit clarification
