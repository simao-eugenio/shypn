...
%DIF ADD revision_2/main_plos_one_revision2.tex   Sun Jul 19 13:35:41 2026
...
\textbf{4. Metabolic-Regulatory Unified Modeling\DIFdelbegin \DIFdel{(Original)}\DIFdelend :} By unifying metabolic and regulatory networks through signal type assignment, commitment thresholds emerge from topology as structural properties computable via $M_{\text{commit}} = \theta(t) + W_s((p_s,t))$. Applied to \textit{B. subtilis} sporulation, the formalism provides a mechanistic account of the Fujita and Losick~\cite{fujita2005} timing-dependent outcome asymmetry (abrupt $\approx 5\%$, gradual $\approx 52\%$): the $\sigma_H$ separatrix at $\approx 1.60\,\mu$M is the analytically predicted commitment boundary (Proposition~\ref{prop:sigmaH_separatrix}), and only the gradual phosphorelay accumulation allows $[\sigma_H]$ to ramp across it. This mechanistic prediction---inexpressible in classical Bio-PN lacking $\Gamma$-threshold semantics---is quantified in Section~\ref{sec:validation}.
...
For the B. subtilis model, the reachability space $\mathcal{R}(M_0)$ partitions into two regions determined by whether $[\sigma_H]$ crosses the commitment separatrix $\theta_{\sigma H} \approx 1.60\,\mu$M:
\begin{align*}
B_{\text{vegetative}} &= \{M \in \mathcal{R}(M_0) : M(\sigma_H) < M_{\text{commit}}(\sigma_H)\} \\
B_{\text{boundary}} &= \{M \in \mathcal{R}(M_0) : M(\sigma_H) = M_{\text{commit}}(\sigma_H)\} \\
B_{\text{sporulation}} &= \{M \in \mathcal{R}(M_0) : M(\sigma_H) > M_{\text{commit}}(\sigma_H) \wedge \text{committed}\}
\end{align*}
where no firing sequence from $B_{\text{vegetative}}$ reaches $B_{\text{sporulation}}$ after commitment transition fires.
\end{proposition}

\begin{proof}
Direct application of Theorem (Irreversible Basin Partition) with $\sigma_H$ as the commitment signal and $\theta_{\sigma H} \approx 1.60\,\mu$M.
When $[\sigma_H]$ crosses the separatrix and the SinR/SinI gate opens, $T_{\mathrm{septation}}$ fires. SigmaH tokens are consumed via signal flow arcs during morphogenetic commitment, and the resulting Septum marking activates the irreversible downstream cascade ($\sigma_F \to \sigma_E \to \sigma_G \to \sigma_K$). With no firing sequence reversing Septum formation, the partition becomes absorbing. Phase space projection onto SigmaF-Forespore plane (Fig~\ref{fig:basin_attraction}) visualizes partition boundaries around the $\sigma_H$ separatrix.
\end{proof}
...
\textbf{Case 1: Phosphorelay kinetics.} Varying KinA autophosphorylation and Spo0F/Spo0B transfer rates by $\pm 20\%$ shifts peak Spo0A$\sim$P by $\approx 15\%$, which changes T32 SinI production proportionally. Empirical sweep: natural sporulation fraction varies $\pm 10\%$ around $52\%$ (range $42\text{--}62\%$). Abrupt fraction remains $< 10\%$ throughout (phosphatase dominates at $N_0=2160\,\mu$M regardless).

\textbf{Case 2: SinR/SinI gate parameters.} Varying T33 sequestration rate $k_{33}$ and T34 synthesis rate $k_{34}$ by $\pm 20\%$ shifts SinR$_{\text{ss}}$ by $\pm 0.3\,\mu$M relative to the $7.5\,\mu$M threshold. Natural fraction varies $40\text{--}64\%$; the gate remains functional throughout.

\textbf{Case 3: $\sigma_H$ feedback strength.} Varying k\_syn\_factor by $\pm 20\%$ shifts the time at which $[\sigma_H]$ crosses $\theta_{\sigma H}$ by $\pm 25\,\text{min}$, not the separatrix value itself. Natural fraction varies $44\text{--}60\%$ (stochastic window shifts slightly).

Union of all cases: natural sporulation $\in [42\%, 62\%]$; abrupt $\in [0\%, 10\%]$. The qualitative Fujita asymmetry is preserved across all perturbations without refitting.
\end{proof}
