#!/bin/bash
# Create single-file LaTeX for bioRxiv with complete structure including references
...
# Add tail sections (acknowledgments, data availability, etc.)
cat tail_sections.tex >> main_single.tex
echo "" >> main_single.tex

# Add bibliography commands and balance
echo "\\balance  % Balance final page columns" >> main_single.tex
echo "" >> main_single.tex
echo "\\bibliographystyle{plainnat}" >> main_single.tex
echo "\\bibliography{references}" >> main_single.tex
echo "" >> main_single.tex

# Close document
echo "\\end{document}" >> main_single.tex

echo "✓ Created main_single.tex with complete structure including bibliography"
