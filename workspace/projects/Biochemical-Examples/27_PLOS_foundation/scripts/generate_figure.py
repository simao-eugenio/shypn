...
ax.axvspan(0, error_barrier, alpha=1.0, color='navy', zorder=0)
ax.axvspan(error_barrier, 5, alpha=1.0, color='firebrick', zorder=0)

# SHYPN prediction curve
ax.plot(atp_range, commitment_prob, 'white', linewidth=2.5, zorder=3)

# Threshold lines
ax.axvline(shypn_threshold, color='darkblue', linestyle='--', linewidth=2, zorder=2)
ax.axvline(exp_threshold, color='red', linestyle='--', linewidth=2, zorder=2)
