...
identificada por varredura de simulação SHPN e compatível com a assimetria
de \textcite{Fujita2005}) e o reclassificava
...
\begin{figure}[!h]
  ...
  \caption{\textbf{Analogia hidráulica da cascata de preempção SHPN}
    ...
    A comporta representa o limiar $M_{\mathrm{commit}} = \theta_{\mathrm{eff}} + W_s$,
    que \emph{separa} $B_{\mathrm{above}}$ ($M(p_s) \geq M_{\mathrm{commit}}$)
    de $B_{\mathrm{below}}$ ($M(p_s) < M_{\mathrm{commit}}$).
    As setas indicam o consumo $W_s$ por disparo em cada camada.
    A linha tracejada laranja ($\theta_{\mathrm{eff}}$) representa o \emph{nível de
    regime pós-comprometimento} --- análogo ao rio retornando às suas margens após
    a cheia: não um leito vazio, mas o nível estável e ativo em que o sistema
    comprometido opera.  É o mínimo residual quando o disparo ocorre a partir de
    exatamente $M_{\mathrm{commit}}$; se $M(p_s) > M_{\mathrm{commit}}$, o residual
    pós-disparo $M'(p_s) = M(p_s) - W_s > \theta_{\mathrm{eff}}$.
    A irreversibilidade é \emph{estrutural}: sem $F_s$ de retorno, a água não
    sobe os degraus.}
...
$M_{\text{commit}}$ exige produção contínua de EPR.  A célula
\emph{bombeia água para o reservatório} contra a comporta: processo
metabólico custoso e reversível.  Enquanto $M(p_s) < M_{\text{commit}}$,
o PreemptionCheck bloqueia todas as transições a jusante.
...
O limiar $\theta_{\mathrm{eff}}$ não é um leito de rio vazio.
É o \textbf{nível de regime}: o nível estável
ao qual o sinal retorna após a cheia, análogo ao rio voltando às margens.
A célula nesse nível está ativa (cascata sigma em execução), não dormente.
