...
é Ackermann-completa~\cite{Czerwinski2021}. Para a biologia, as Redes de Petri clássicas apresentam
...
forte, e ausência de atribuição de compartimento termodinâmico para espécies.
...
a composição bioquímica. Nenhum desses sistemas associa formalmente cada espécie
a um compartimento celular (volume termodinâmico) ou rastreia fórmulas
elementares como \ce{C6H12O6} para verificação estequiométrica automatizada.
...
\textbf{Atribuição de compartimento e fronteira termodinâmica.}
As Redes de Petri clássicas tratam todos os lugares como habitantes de um único
espaço homogêneo, sem noção de volume, membrana ou domínio termodinâmico.
Nenhum formalismo existente associa formalmente cada espécie ao compartimento
celular que a contém (citoplasma, núcleo, mitocôndria) para converter tokens
em concentrações via $c = N / (N_A \cdot V_{C(p)})$ e definir as fronteiras
onde ocorre produção de entropia.
A contribuição desta tese é a função de atribuição de compartimento
$C: P \to \mathcal{C}$ como componente formal da 13-tupla, que conecta
cada espécie a um volume $V_{C(p)}$ e estabelece as fronteiras termodinâmicas
do sistema.

\textbf{Identidade química dos tokens.}
As Redes de Petri Coloridas oferecem sistemas de tipos abstratos que distinguem
glicose de frutose, mas os tokens carecem de fórmula elementar. Nenhum
formalismo existente rastreia cadeias como \ce{C6H12O6} para habilitar
verificação estequiométrica automatizada ou verificações de conservação de
átomos. Nas SHPN, a identidade química é tratada como \textbf{metadado
descritivo} externo à tupla formal: cada lugar é identificado pelo seu
nome/ID, e anotações de fórmula (SMILES, InChI, ChEBI) são opcionais.
Na plataforma SHYpn, esse campo
...
carrega parâmetros $(K, n, \varepsilon)$ derivados de $\Gamma$ que determinam o limiar efetivo
...
dimensões de capacidade avaliadas. \textbf{Tokens} designa a atribuição de compartimento ($C: P \to \mathcal{C}$,
fronteira termodinâmica) e a anotação de identidade química via ChEBI
por nome de composto (cobertura parcial; sem entrada manual de
