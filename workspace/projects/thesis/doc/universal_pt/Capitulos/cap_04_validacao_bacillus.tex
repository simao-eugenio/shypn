...
Os sete painéis mostram a montagem morfológica sequencial:
Septo ($t_{\text{med}} \approx 334$~min) e Pré-esporo surgem primeiro;
Célula-mãe, Córtex, Casca Interna e Casca Externa formam-se em seguida;
Esporo Maduro materializa-se em $t \approx 338$~min
(cf.\ Tabela~\ref{tab:v6-factorial}).
A linha tracejada vertical em $t \approx 17$~min marca a ignição média
de $\sigma^H$ (cruzamento de $\theta_{\sigma H}=1{,}60$~µM), detonando
o programa estrutural a jusante.
...
mediana de $\sigma^H$ em $1{,}60\,\mu$M).
...
($t \approx 329$~min). Linha tracejada vermelha: separatriz de $\sigma^H$.
...
caráter hierárquico e unidirecional da cascata SHPN.
...
\textit{Superior} --- Spo0A$\sim$P: fosforretransmissão estocástica;
...
\textit{Inferior} --- $\sigma^F$: programa de execução; silencioso nas réplicas
