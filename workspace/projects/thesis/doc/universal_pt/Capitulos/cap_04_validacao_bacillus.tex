...
subsequentemente.
O limiar de $\sigma^H$ que determina o comprometimento celular
--- a separatriz que discrimina acumulação gradual de Spo0A${\sim}$P
(${\approx}\,52\%$ de esporulação) de indução constitutiva abrupta de Spo0A*
(${\approx}\,5\%$) --- foi identificado por \textcite{Fujita2005}.
...
(40~lugares, 36~transições, 114~arcos).
Círculos~($\bigcirc$): lugares biológicos de sinal~$\Psi$;
...
micromolares, compatíveis com as medidas de microscopia de
fluorescência~\cite{Fujita2005}.
...
\midrule
Lugares $P$ & 40 no total: 28 lugares de sinal biológico~$\Psi$
             (fosforretransmissão, fatores sigma, reservatórios
             energéticos, compartimentos, estruturas e competição
             por biofilme), 9 lugares de parâmetro~$\square$
             ...
             \texttt{INTERVENTION\_TIME\_S}, lidos por eventos apenas)
...
com as escalas temporais medidas por microscopia de
fluorescência~\cite{Fujita2005}.
...
\textbf{Painéis superiores:} reguladores-mestres Spo0A$\sim$P (vermelho)
e $\sigma^H$ (magenta); a linha tracejada horizontal marca a separatriz
$\theta_{\sigma H} = 1{,}60$~µM.
\textbf{Painéis inferiores:} montagem morfológica sequencial — Septum,
Foresporo, Córtex, Casca Interna, Casca Externa e Esporo Maduro.
Curvas: média sobre réplicas; bandas sombreadas: $\pm 1$~dp.
A linha tracejada vertical em $t = 334$~min indica a mediana do
comprometimento morfológico (primeiro aparecimento de Septum nas
réplicas esporulantes).}
...
\caption{Paisagem de potencial epigenético da esporulação de
...
\caption{Cascata de pré-empção da esporulação de \textit{B.~subtilis}:
...
\textbf{Faixas, de cima para baixo:} Spo0A$\sim$P (vermelho),
$\sigma^H$ (magenta) e $\sigma^F$ (verde-azulado).
Réplicas esporulantes a vermelho escuro; réplicas vegetativas a cinzento.
A linha tracejada amarela marca a separatriz
$\theta_{\sigma H} = 1{,}60$~µM; a linha tracejada vertical vermelha
indica $t_{\text{commit}} = 300$~min (mediana do cruzamento).
A trajetória média é destacada a vermelho escuro espesso.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_cascade_weir_analogy.pdf}
  \caption{\textbf{Analogia hidráulica da cascata de preempção SHPN.}
    O perfil de um açude com comportas em degraus ilustra os elementos
    centrais do mecanismo de preempção hierárquica.
    O reservatório (esquerda) representa os tokens de sinal acumulados
    $M(p_s)$; a comporta de controle corresponde ao limiar de comprometimento
    $M_{\mathrm{commit}} = \theta + W_s$.
    Cada degrau é uma camada $\lambda$ da hierarquia: Camada~0
    (KinA$_{\mathrm{kinase}}$), Camada~1 (Spo0A${\sim}$P),
    Camada~2 ($\sigma^H$), Camada~3 ($\sigma^F$/$\sigma^E$).
    As setas indicam o consumo de tokens $W_s$ em cada disparo
    (queda de nível), análogo ao fluxo de água passando de um degrau
    ao seguinte.
    A ausência de arco $F_s$ de retorno corresponde à impossibilidade
    hidráulica de a água subir os degraus sem uma bomba externa:
    a irreversibilidade é \emph{estrutural}, não apenas energética.
    O patamar $\theta$ (linha tracejada inferior) é o fundo de bacia
    que separa $B_{\mathrm{above}}$ de $B_{\mathrm{below}}$.}
