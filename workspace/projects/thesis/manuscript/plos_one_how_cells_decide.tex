...
Trajectory analysis (G5 tier) reveals two additional features.
\emph{Critical slowing down} is confirmed: mean commitment time peaks at
$\approx 345\,\min$ near $N_c$ (compared to $311\,\min$ at $N_0 = 1200\,\mu$M
and shorter for rare-event commitments at $N_0 > 1600\,\mu$M).
\emph{Threshold-crossing mechanism}: the mean $\sigma_H$ peak is $1.50\,\mu$M
at $N_c$ --- below the architectural threshold $\theta_{\sigma H} = 1.60\,\mu$M.
The sporulation fraction equals the probability that a stochastic $\sigma_H$
trajectory transiently exceeds the threshold, directly linking $N_c$ to the
architecture of the $\Gamma$ gate.
