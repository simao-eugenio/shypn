...
\begin{table}[ht]
\centering
\caption{\textbf{NET thermodynamic tests} (predecessor v7 model;
  v9 re-runs in preparation).  All sweeps: $\tau_{\max} = 0.01$~s,
...
\caption{\textbf{Fujita Protocol sweep results} (v9 model, 6-h horizon,
  ...
  replicates only).
  F4 (FUJITA-4, fine nutrient sweep) is in progress at time of writing.}
...
\textbf{Q3 status: fine sweep in progress.}
FUJITA-4 (fine $N_0$ sweep, 200 replicates, 16 conditions) was
dispatched but had not completed at time of writing.  Preliminary results
from F1/F2 place the $50\%$ crossover between $N_0 = 1440$ and
$1600\,\mu$M.  The NET~4 critical exponents ($\gamma_{\mathrm{sub}} \approx 2.4$,
$\gamma_{\mathrm{sup}} \approx 4.6$, v7 model) indicate a noise-driven
universality class with asymmetric basin curvature; the resulting
near-equal basin depths over a wide nutrient range ($\approx 38$--$66\%$
across all F1/F2 CH conditions) explain why the Fujita $\approx 52\%$
is not a special value but the thermodynamically natural outcome
near a noise-driven bifurcation without parameter bias.
