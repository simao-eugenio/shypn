    ...
    def _subdivide_wide_layers(self, layers: List[List[str]], max_per_layer: int = 15) -> List[List[str]]:
        ...
        for layer_idx, layer in enumerate(layers):
            if len(layer) <= max_per_layer:
                ...
                self.logger.info(f"Subdividing layer {layer_idx} with {len(layer)} species into {num_sublayers} sub-layers")
                subdivided_count += 1
        ...
        if subdivided_count > 0:
            self.logger.info(f"Subdivided {subdivided_count} wide layers: {len(layers)} → {len(new_layers)} total layers")
        return new_layers
