...
class ExperimentManager:
    ...
    # ── CLI sweep config export ──────────────────────────────────────

    def export_sweep_config(
        self,
        filepath,
        model_path: str = '',
        replicates: int = 200,
        ...
        If the current snapshots were generated by a single-parameter
        sweep, the config is exported as ``mode: single``.  If multiple
        parameters were swept (factorial), it exports ``mode: factorial``.
        Otherwise it falls back to ``mode: snapshots`` with the full
        snapshot list embedded.

        The exported file can be run directly::

            python -m shypn.cli.sweep --project <project_folder> \\
                --sweep <exported_file>

        Args:
            filepath: Destination JSON path.
            model_path: Relative path to the .shy model (from project root).
            replicates: Number of stochastic replicates per condition.
    ...
    def _infer_sweep_config(self, sim_block: dict) -> dict:
        """Try to infer a compact sweep mode from the current snapshots."""
        if not self.snapshots:
            return {**sim_block, 'mode': 'snapshots', 'snapshots': []}

        # Check if snapshots have swept_parameter metadata
        swept = [
            getattr(s, 'swept_parameter', None)
            for s in self.snapshots
        ]
        has_sweep_info = all(sp is not None for sp in swept)

        if has_sweep_info and swept:
            # Single parameter: all swept_parameter entries have same 'id'
            ids = set(sp.get('id', '') for sp in swept)
            types = set(sp.get('type', '') for sp in swept)

            if len(ids) == 1 and '; ' not in next(iter(ids)):
                # Single-parameter sweep
                param_id = next(iter(ids))
                param_type = next(iter(types))
                values = [sp.get('value') for sp in swept]
                return {
                    **sim_block,
                    'mode': 'single',
                    'parameter': {
