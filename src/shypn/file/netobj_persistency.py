...
class NetObjPersistency:
    ...
    def __init__(self, parent_window: Optional[Gtk.Window] = None, models_directory: Optional[str] = None):
        ...
        if models_directory is None:
            ...
            # This file is in: src/shypn/file/netobj_persistency.py
        ...
        if not os.path.exists(self.models_directory):
            try:
                ...
                print(f"[Persistency] Created models directory: {self.models_directory}")
            except Exception as e:
                print(f"[Persistency] Warning: Could not create models directory: {e}")
        
        # Last used directory starts at models directory
    ...
    def mark_dirty(self) -> None:
        ...
        if not self._is_dirty:
            ...
            print("[Persistency] Document marked as dirty (has unsaved changes)")
    ...
    def mark_clean(self) -> None:
        ...
        if self._is_dirty:
            ...
            print("[Persistency] Document marked as clean (no unsaved changes)")
    ...
    def save_document(self, document, save_as: bool = False, is_default_filename: bool = False) -> bool:
        ...
        # ALWAYS prompt if:
        # 1. Save As operation
        # 2. No filepath exists (new document)
        # 3. Document has default filename (unsaved state flag)
        needs_prompt = save_as or not self.has_filepath() or is_default_filename
        
        print(f"[Persistency] needs_prompt = {needs_prompt} (save_as={save_as} OR not has_filepath={not self.has_filepath()} OR is_default_filename={is_default_filename})")
        
        if needs_prompt:
            print("[Persistency] Showing file chooser dialog")
            ...
            print(f"[Persistency] Saving directly to: {self.current_filepath}")
    ...
    def load_document(self):
        ...
        if not filepath:
            print("[Persistency] Load cancelled by user")
            return None, None
        ...
        try:
            ...
            print(f"[Persistency] Loaded from {filepath}")
            print(f"[Persistency] Places: {len(document.places)}, " +
            ...
            print(f"[Persistency] Load error: {e}")
            import traceback
            traceback.print_exc()
    ...
    def check_unsaved_changes(self) -> bool:
        ...
        dialog.format_secondary_text(
            f"Document '{self.get_display_name()}' has unsaved changes.\n" +
        ...
        if response == Gtk.ResponseType.YES:
            # User wants to save - need document to save
            # This method is typically called before an operation,
            # so the caller should handle the actual save
            return True
    ...
    def new_document(self) -> bool:
        ...
        print("[Persistency] New document created")
        return True
...
def create_persistency_manager(parent_window: Optional[Gtk.Window] = None, 
                               models_directory: Optional[str] = None) -> NetObjPersistency:
