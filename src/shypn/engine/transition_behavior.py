...
class TransitionBehavior(ABC):
    ...
    def _get_place(self, place_id):
        ...
        # Model stores places as a list, not a dict - must iterate
        # Normalize both sides to strings for comparison
        place_id_str = str(place_id)
        
        for place in self.model.places:
            if str(place.id) == place_id_str:
                return place
        
        # Fallback: if place_id is string like "P101", also try matching
        # against numeric part (handles legacy int IDs)
        if isinstance(place_id, str) and place_id.startswith('P'):
            try:
                numeric_id = int(place_id[1:])  # "P101" -> 101
                for place in self.model.places:
                    if place.id == numeric_id:
                        return place
            except ValueError:
                pass
