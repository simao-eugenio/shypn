#!/usr/bin/env python3
"""Test script to verify timed sink transition behavior."""

# Test: Create a simple net with a place, timed transition, and check if sink works
# Expected: Timed sink should consume tokens but not produce any
...
class MockTransition:
    ...
    def __init__(self, id, name):
        ...
        self.is_sink = True  # THIS IS THE KEY
        self.properties = {
            'earliest': 1.0,
            'latest': 1.0
        }
...
print("=" * 60)
print("Testing Timed Sink Transition")
print("=" * 60)

# Setup: Place P1 with 5 tokens -> Timed Transition T1 (sink)
model = MockModel()

place1 = MockPlace(1, 5)
model.add_place(place1)

transition1 = MockTransition(1, "T1")
model.add_transition(transition1)

# Arc from Place to Transition (input arc for the transition)
arc1 = MockArc(1, place1, transition1, 2)  # Consumes 2 tokens
model.add_arc(arc1)

print(f"\nInitial State:")
print(f"  Place P1: {place1.tokens} tokens")
print(f"  Transition T1: is_sink={transition1.is_sink}, type={transition1.transition_type}")
...
if can_fire:
    ...
    print(f"\nExpected: 3 tokens (5 - 2)")
    print(f"Actual: {place1.tokens} tokens")
    
    if place1.tokens == 3:
        print("\n✅ TEST PASSED: Sink correctly consumed tokens without producing")
    else:
